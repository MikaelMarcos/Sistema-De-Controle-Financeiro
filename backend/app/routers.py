...
# --- Função Auxiliar de Análise de Orçamento ---
...
def read_budget_groups(
    *, 
    ...
    return session.exec(select(BudgetGroup).where(BudgetGroup.user_id == user.id)).all()
    ...
    *, 
    ...
    for i in range(installments_total):
        ...
        db_expense = Expense(
            ...
            date=installment_date, paid=is_paid,
            budget_group_id=expense_in.budget_group_id, category_id=expense_in.category_id,
            goal_id=expense_in.goal_id, credit_card_id=expense_in.credit_card_id,
        ...
        if db_expense.goal_id and current_installment == 1:
            ...
            if not goal or goal.user_id != user.id: raise HTTPException(status_code=404, detail="Meta não encontrada")
    ...
    try: session.commit()
    except exc.IntegrityError: session.rollback(); raise HTTPException(status_code=500, detail="Erro ao salvar.")
...
def read_expenses(
    *, 
    ...
    statement = select(Expense).where(Expense.user_id == user.id).options(
        joinedload(Expense.budget_group), joinedload(Expense.category),
        joinedload(Expense.goal), joinedload(Expense.credit_card)
    )
    if month and year:
...
def delete_expense(*, session: Session = Depends(get_session), user: User = Depends(get_current_user), expense_id: int):
    ...
    if not expense or expense.user_id != user.id: raise HTTPException(status_code=404, detail="Despesa não encontrada")
...
def create_income(*, session: Session = Depends(get_session), user: User = Depends(get_current_user), income: IncomeCreate):
    ...
    session.add(db_income); session.commit(); session.refresh(db_income)
    return db_income
...
def delete_income(*, session: Session = Depends(get_session), user: User = Depends(get_current_user), income_id: int):
    ...
    if not income or income.user_id != user.id: raise HTTPException(status_code=404, detail="Entrada não encontrada")
...
def read_goals(*, session: Session = Depends(get_session), user: User = Depends(get_current_user)):
    goals = session.exec(select(Goal).where(Goal.user_id == user.id)).all(); goals_read_list = []
    for goal in goals:
...
def update_goal(*, session: Session = Depends(get_session), user: User = Depends(get_current_user), goal_id: int, goal: GoalCreate):
    ...
    if not db_goal or db_goal.user_id != user.id: raise HTTPException(status_code=404, detail="Meta não encontrada")
...
def delete_portfolio_holding(*, session: Session = Depends(get_session), user: User = Depends(get_current_user), holding_id: int):
    ...
    if not holding or holding.user_id != user.id: raise HTTPException(status_code=404, detail="Ativo não encontrado")
...
def update_credit_card(*, session: Session = Depends(get_session), user: User = Depends(get_current_user), card_id: int, card: CreditCardCreate):
    ...
    if not db_card or db_card.user_id != user.id: raise HTTPException(status_code=404, detail="Cartão não encontrado")
...
def delete_credit_card(*, session: Session = Depends(get_session), user: User = Depends(get_current_user), card_id: int):
    ...
    if not card or card.user_id != user.id: raise HTTPException(status_code=404, detail="Cartão não encontrado")
...
def pay_credit_card_fatura(
    *,
    ...
    try: session.commit()
    except Exception as e: session.rollback(); raise HTTPException(status_code=500, detail=f"Erro: {e}")
